...
impl Kvm {
    ...
    pub fn write_buf(&self, addr: usize, buf: &[u8]) -> Result<()> {
        let w = unsafe {
            kvm_kwrite(
                self.kvm,
                addr,
        ...
        if w == -1 {
            let e = std::io::Error::last_os_error();
            bail!("could not write 0x{addr:x}: {e}");
        } else if w != buf.len() as isize {
            bail!("write {w} bytes, but wanted {}", buf.len());
        } else {
            Ok(())
        }
    }
    ...
    pub fn write_usize(&self, addr: usize, value: usize) -> Result<()> {
        let mut buf = [0u8; size_of::<usize>()];

        self.write_buf(addr, &buf)?;
